...
| U7 | "Advanced options" panel | 🟡 | **Seeded by B1 (2026-06-13):** an "Advanced options" `<details>` reveal hosts the caption-style `<select>` (Hormozi default / word-pop). **B5 (2026-06-14)** added an aspect-ratio `<select>` (9:16 / 1:1 / 4:5 / 16:9, 9:16 default) into the same reveal. Still to add: music toggle (B6), silence-trim (B2). |
